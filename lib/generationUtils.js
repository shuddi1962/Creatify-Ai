...
export async function getApiKey() {
  ...
  // Check database for admin-configured Muapi key (cached)
  if (cachedDbKey) return cachedDbKey
  try {
    const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL || (typeof window !== 'undefined' ? window.ENV?.SUPABASE_URL : null)
    const supabaseKey = process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY || (typeof window !== 'undefined' ? window.ENV?.SUPABASE_ANON_KEY : null)
    if (supabaseUrl && supabaseKey) {
      const res = await fetch(
        `${supabaseUrl}/rest/v1/api_keys?select=encrypted_key&provider=eq.muapi&is_active=eq.true&limit=1`,
        { headers: { 'apikey': supabaseKey } }
      )
      if (res.ok) {
        const data = await res.json()
        if (data?.length > 0 && data[0].encrypted_key) {
          cachedDbKey = data[0].encrypted_key
          return cachedDbKey
        }
      }
    }
