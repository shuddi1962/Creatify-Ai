...
const VALID_DURATIONS = [5, 6, 8, 10, 12, 14, 16, 18, 20]
...
export async function generateVideo({ model, prompt, aspect_ratio, duration, quality, image_url }) {
  ...
  if (duration) {
    ...
    const safe = VALID_DURATIONS.filter(v => v <= 8)
    payload.duration = VALID_DURATIONS.includes(d) ? d : (safe.length > 0 ? safe[safe.length - 1] : 8)
  }
