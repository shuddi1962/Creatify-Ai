...
export async function generateImage({ model, prompt, aspect_ratio, quality, style }) {
  ...
  if (requestId) {
    ...
    for (let attempt = 0; attempt < 60; attempt++) {
      ...
      try {
        const pollRes = await fetchWithTimeout(`/api/v1/predict/results/${requestId}`, {}, 10000)
...
export async function generateVideo({ model, prompt, aspect_ratio, duration, quality, image_url }) {
  ...
  if (requestId) {
    ...
    for (let attempt = 0; attempt < 300; attempt++) {
      await new Promise(r => setTimeout(r, 2000))
      try {
        const pollRes = await fetchWithTimeout(`/api/v1/predict/results/${requestId}`, {}, 15000)
